 ...
 *
 * In Slice 4 the mock endpoint is replaced by the real Square POS integration; the on-chain
...
const reportParams = parseAbiParameters('address merchant, address payer, uint256 amount, uint8 purpose')
...
const onCronTrigger = (runtime: Runtime<Config>, _payload: CronPayload): string => {
  const backendToken =
    runtime.config.backendInternalToken?.trim() || getSecretValue(runtime, 'BACKEND_INTERNAL_TOKEN')
  ...
  for (const checkout of pending.checkouts) {
    try {
      ...
      processed += 1
      runtime.log(`Settlement success for checkout ${checkout.checkoutId}: ${txHash}`)
    } catch (error) {
      sendJson(runtime, {
        url: `${runtime.config.backendBaseUrl}${runtime.config.onchainResultPath}`,
        method: 'POST',
